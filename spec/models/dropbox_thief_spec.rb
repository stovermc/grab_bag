...
describe DropboxThief do

  before :each do
    @token = ENV['DROPBOX_TOKEN']
    @client = DropboxApi::Client.new(@token)
    @dropbox_thief = DropboxThief.new(@client, "dbid:AADpQYmD5FJPCJzItJQ8i95Cc-99TcvIgbM")
  end

  it " #account_info" do
    account = @dropbox_thief.account_info
  ...
  it " #space_allocated" do
    space = @dropbox_thief.space_allocated

    expect(space).to eq(2.6)
  end

  it "#space_used" do
    space = @dropbox_thief.space_used

    expect(space).to eq(1.62)
  end

  it "#home_folder_contents" do
    home_folders = @dropbox_thief.home_folder_contents
